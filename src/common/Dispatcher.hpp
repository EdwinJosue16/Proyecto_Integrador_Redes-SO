...
class Dispatcher : public Consumer<DataType>
{
  ...
  protected:
	...
	Queue<DataType>& fromQueue = Consumer<DataType>::queue;
	/// This thread will consume from its queue
	std::map<KeyType, Queue<DataType>*> toQueues;

  public:
	/// Constructor
	Dispatcher(const DataType& stopCondition = DataType())
	...
	virtual void consume(const DataType& data) override
	{
	...
	/// Override this method to extract the key from a data stored in the fromQueue
